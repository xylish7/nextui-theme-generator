...
import { initialConfig } from "../shared/constants";
import { ConfigColors, Config, ConfigLayout, ThemeType } from "../shared/types";
...
export default function ThemeBuilderProvider({
  children,
}: ThemeBuilderProviderProps) {
  const storageConfig = storage.getConfiguration() ?? initialConfig;
  const [config, setConfig] = useState<Config>(storageConfig);

  const setConfiguration = useCallback(
    (newConfig: Config, theme: ThemeType, sync: boolean) => {
      setConfig((prev) =>
        sync
          ? newConfig
          : {
